...
export default {
  ...
  setup() {
    ...
    const loadData = async () => {
      try {
        ...
        const [quarterlyResponse, monthlyResponse] = await Promise.all([
          api.getReports('quarterly'),
          api.getReports('monthly-trends')
        ])
    ...
    const calculateSummaryStats = () => {
      ...
      let bestRev = 0
      let bestQ = ''
      quarterlyData.value.forEach(quarter => {
        if (quarter.total_revenue > bestRev) {
          bestRev = quarter.total_revenue
          bestQ = quarter.quarter
        }
      })
    ...
    const getChangeValue = (current, previous) => {
      ...
      if (change > 0) {
        return '+' + formatCurrency(change, selectedCurrency.value)
      } else if (change < 0) {
        return '-' + formatCurrency(Math.abs(change), selectedCurrency.value)
      }
